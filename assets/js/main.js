/* ============================================
   B1 COURS INTERACTIFS — main.js (shared)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ChapterTracker = (() => {
        // Detect course from URL path (e.g. /algo/chapitres/chapitre3.html)
        const courseMatch = window.location.pathname.match(/\/(algo|c_cpp|python|reseaux|stats|fbd|merise)\//);
        if (!courseMatch) return;
        const course = courseMatch[1];
        const KEY = `b1_${course}_completed`;
    ...
    const CodeCopy = (() => {
        document.querySelectorAll('.code-block').forEach(block => {
            ...
            btn.addEventListener('click', () => {
                const code = block.textContent.replace(/Copier|Copié !/, '').trim();
