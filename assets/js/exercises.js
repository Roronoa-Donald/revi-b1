/* ═══════════════════════════════════════════════════
   B1 COURS INTERACTIFS — Exercise Page Engine (shared)
   For exercices/exercices.html pages
   Usage: RD_Exercises.init(exerciseList)
   ═══════════════════════════════════════════════════ */
(function () {
    'use strict';

    const STORAGE_KEY = 'b1_exercises_revealed';
    ...
    function saveState() {
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify([...revealedAnswers])); } catch(e) {}
    }

    function initExercises(exList, storageKeySuffix) {
        exercises = exList;
        if (storageKeySuffix) {
            // Allow per-course storage key
            Object.defineProperty(window, '_exStorageKey', { value: `b1_${storageKeySuffix}_exercises`, writable: true });
        }
        loadState();
        renderFilters();
        render();
    ...
    function render() {
        ...
        if (countEl) countEl.textContent = `${filtered.length} exercice${filtered.length > 1 ? 's' : ''}`;
        ...
        container.innerHTML = filtered.map((ex, i) => {
            ...
            const levelBadge = ex.level === 'exam'
                ? '<span style="background:#ef4444;color:#fff;padding:2px 8px;border-radius:4px;font-size:.7rem;margin-left:.5rem">EXAMEN</span>'
                : '<span style="background:var(--accent);color:#fff;padding:2px 8px;border-radius:4px;font-size:.7rem;margin-left:.5rem">BASE</span>';
            return `
                <div class="ex-card" data-idx="${globalIdx}">
                    <div class="ex-header">
                        <span class="ex-num">Exercice ${i + 1}</span>
                        <span style="font-size:.75rem;color:var(--text-muted)">${ex.catLabel || ex.cat || ''}</span>
                        ${levelBadge}
                    </div>
                    <div class="ex-body">
        ...
        container.querySelectorAll('.ex-toggle').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                else { revealedAnswers.add(idx); if (typeof GameEngine !== 'undefined') GameEngine.addXP(5); }
                saveState();
                render();
            });
        });
